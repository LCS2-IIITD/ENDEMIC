...
class Classifier(nn.Module):
    def __init__(self, config):
        ...
        if config.brnn:
            seq_in_size *= 2
        if self.config.use_addn:
            layers1 = [_linear(config.addn_dim, 64, config.init_scalar),
                      nn.ReLU(),
                      nn.Dropout(0.3),
                      _linear(64, 256, config.init_scalar),
                      nn.ReLU()]
            layers2 = [_linear(1024, 1024, config.init_scalar),
                      nn.ReLU(),
                      nn.Dropout(0.3)]
            layers3 = [_linear(256, 256, config.init_scalar),
                      nn.ReLU(),
                      nn.Dropout(0.3),
                      _linear(256, 512, config.init_scalar),
                      nn.ReLU()]
        ...
        if config.down_projection:
            self.down_projection = _linear(seq_in_size,
                                           256,
                                           config.init_scalar)
            self.down_projection = _linear(256,
                                           config.d_down_proj,
                                           config.init_scalar)
            self.act = nn.Sigmoid()
            seq_in_size = config.d_down_proj
    ...
    def attention_net(self, a, b):
        return torch.bmm(
            a.transpose(1,2), 
            F.softmax(torch.bmm(a,
                                b.reshape(1, -1, 
                                          self.config.d_hidden*2).squeeze(0).unsqueeze(2)).squeeze(2)).unsqueeze(2)).squeeze(2)

    def forward(self, x, y, final_state):
        if True:
        ...
        if self.config.use_addn:
            y = self.addn_model1(y.float())
#             s_o1, s_o2 = apply_cross_stitch(sent_output, y)
#             y = self.addn_model3(s_o2)
            sent_output = self.addn_model2(sent_output)
#             s_o1, s_o2 = apply_cross_stitch(s_o1, y)
            y = self.addn_model3(y)
#             y = s_o2
            sent_output = torch.cat((sent_output, y.float()), dim=1)
...
def at_loss(embedder, encoder, clf, batch, addn_batch, perturb_norm_length=5.0):
    embedded = embedder(batch)
    embedded.retain_grad()
    one, two = encoder(embedded, batch)
    ce = F.cross_entropy((clf(one, addn_batch, two)), batch.labels)
    ce.backward()
    ...
    loss = F.cross_entropy(clf(one, addn_batch, two), batch.labels)
    return loss
...
def vat_loss(embedder, encoder, clf, batch, addn_batch, perturb_norm_length=5.0,
    ...
    for ip in range(Ip):
        ...
        p_d_logit = clf(one, addn_batch, two)
    ...
    p_adv_logit = clf(one, addn_batch, two)
