...
class Classifier(nn.Module):
    def __init__(self, config):
        ...
        if config.brnn:
            seq_in_size *= 2
        if self.config.use_addn and False:
            ...
            layers2 = [_linear(256, 256, config.init_scalar),
                      nn.GELU(),
                      nn.Dropout(0.3)]
            layers3 = [_linear(256, 512, config.init_scalar),
                      nn.GELU()]
    ...
    def forward(self, x, y, final_state):
        ...
        if self.config.use_addn and False:
            ...
            y = self.addn_model2(s_o2)
            s_o1 = self.addn_model2(s_o1)
#             s_o1, s_o2 = apply_cross_stitch(s_o1, y)
#             y = self.addn_model3(s_o2)
            sent_output = torch.cat((s_o1, y.float()), dim=1)
...
class Embedder(nn.Module):
    ...
    def forward(self, batch):
        outputs = self.embedder(batch.word_ids)
