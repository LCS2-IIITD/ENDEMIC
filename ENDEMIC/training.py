...
import batch_utils
import utils
from model3 import LSTMEncoder, Classifier, Discriminator, SequenceCriteria, \
...
#             self.embedder.load_state_dict(dict_['state_dict_embedder'])
    ...
    def __call__(self, train, dev, test, unlabel,
                 addn, addn_un, addn_test,
                 ek, ek_t, ek_u,
                 graph_embs, graph_embs_t, graph_embs_u,
                 addn_test_fr=None, addn_test_f=None, addn_test_r=None,
                 mode="train", checkPth=None):
        ...
        if(mode == "train"):
            ...
            if self.config.behaviour_test:
                self._evaluate(test, addn_test, ek_t, graph_embs_t, addn_test_fr, addn_test_f, addn_test_r)
            else:
    ...
    def _run_epoch(self, train_data, dev_data, unlabel_data, 
        ...
        for batch_index, train_batch_raw in enumerate(train_iter):
            ...
            if self.config.lambda_vat > 0 or self.config.lambda_ae > 0 or self.config.lambda_entropy:
                ...
                ek_batch_unlab = retAddnBatch(ek_u, memory_bank_unlabel.shape[0], sofar_1).cuda()
                graph_embs_unlab = retAddnBatch(graph_embs_u, 
                                                memory_bank_unlabel.shape[0], sofar_1).cuda()
            ...
            ek_batch = retAddnBatch(ek, memory_bank_train.shape[0], sofar).cuda()
            graph_embs_batch = retAddnBatch(graph_embs, memory_bank_train.shape[0], sofar).cuda()
            sofar += addn_batch.shape[0]
            if sofar >= ek.shape[0]:
                sofar = 0
            pred = self.clf(memory_bank_train, addn_batch, ek_batch, enc_final_train, graph_embs_batch)
            ...
            if self.config.lambda_vat > 0:
                ...
                if self.config.inc_unlabeled_loss:
                    if memory_bank_unlabel.shape[0] != ek_batch_unlab.shape[0]:
                        print(f'Skipping; Unequal Shapes: {memory_bank_unlabel.shape} and {ek_batch_unlab.shape}')
                        continue
                    else:
            ...
            if self.config.lambda_entropy > 0:
                ...
                if self.config.inc_unlabeled_loss:
                    lentropy_unlabel = entropy_loss(self.clf(memory_bank_unlabel, addn_batch_unlab, 
                                                             ek_batch_unlab, enc_final_unlabel, graph_embs_unlab))
                    if self.config.unlabeled_loss_type == "AvgTrainUnlabel":
            ...
            if self.global_steps % self.config.eval_steps == 0:
                cm_, accuracy, prc_dev = self._run_evaluate(dev_data, addn_dev, ek_t, graph_embs_t)
...
#                 self.embedder.train()
        ...
        cm, train_accuracy, _ = self._run_evaluate(train_data, addn_data, ek, graph_embs)
        self.logger.info("- Train accuracy  {}".format(train_accuracy))
        pprint.pprint(cm.get_all_metrics())

        cm, dev_accuracy, _ = self._run_evaluate(dev_data, addn_dev, ek_t, graph_embs_t)
    ...
    def _predict_batch(self, cm, batch, addn_batch, ek_batch, graph_embs_batch):
#         self.embedder.eval()
        self.encoder.eval()
        self.clf.eval()
        one, two = self.encoder(self.embedder(batch), batch)
        ...
        torch.save(self.clf.co_attn_1.seq_len_weights, 'co_attn_weights_eval.pth')
        accuracy = self.get_accuracy(cm, pred.data, batch.labels.data)
    ...
    def _run_evaluate(self, test_data, addn_test, ek_t, graph_embs_t):
        ...
        for batch_index, test_batch in enumerate(test_iter):
            ...
            graph_embs_batch = graph_embs_t[batch_index*15:(batch_index+1)*15]
            test_batch = batch_utils.seq_pad_concat(test_batch, -1)
            try:
                pred, acc = self._predict_batch(cm, test_batch, addn_batch, ek_batch, graph_embs_batch)
            except:
                continue
    ...
    def _train(self, train_data, dev_data, unlabel_data, addn_data, addn_data_unlab, addn_dev, ek, ek_t, ek_u, graph_embs, graph_embs_t, graph_embs_u):
...
#             random.shuffle(unlabel_data)
    ...
    def _evaluate(self, test_data, addn_test, ek_t, graph_embs_t, 
                  addn_test_fr=None, addn_test_f=None, addn_test_r=None, mode="train"):
        ...
        if(mode=="train"):
            self._load_model()
        _, accuracy, prc_test = self._run_evaluate(test_data, addn_test, ek_t, graph_embs_t)
        if self.config.behaviour_test:
            addn_test_fr = addn_test_fr.cuda()
            addn_test_f = addn_test_f.cuda()
            addn_test_r = addn_test_r.cuda()
            _, accuracy_fr, prc_test_fr = self._run_evaluate(test_data, addn_test_fr, ek_t, graph_embs_t)
            _, accuracy_f, prc_test_f = self._run_evaluate(test_data, addn_test_f, ek_t, graph_embs_t)
            _, accuracy_r, prc_test_r = self._run_evaluate(test_data, addn_test_r, ek_t, graph_embs_t)
        ...
        if self.config.behaviour_test:
            self.logger.info("- behaviour test accuracy - fr {}".format(accuracy_fr))
            self.logger.info("- behaviour test accuracy - f {}".format(accuracy_f))
            self.logger.info("- behaviour test accuracy - r {}".format(accuracy_r))