...
class Discriminator(nn.Module):
    def __init__(self, config):
        ...
        if self.config.use_addn:
            seq_in_size += config.addn_dim
        if config.brnn:
            seq_in_size *= 2
...
class Co_Attention(nn.Module):
    def __init__(self, a_dim, b_dim, k):
        ...
        self.w_sg = nn.Linear(a_dim, b_dim)
        self.w_hs = nn.Linear(k, 1)
        self.w_hg = nn.Linear(k, 1)
        self.w_s = nn.Linear(a_dim, k)
        self.w_g = nn.Linear(b_dim, k)
        
    def forward(self, a, b):
        """
        a (source embeddings, S) -> (bsz, a_num_nodes, a_dim)
        b (interaction embeddings, G) -> (bsz, b_num_nodes, b_dim)
        
        w_sg = nn.Linear(a_dim, b_dim)
        w_hs = nn.Linear(k, 1)
        w_hg = nn.Linear(k, 1)
        w_s = nn.Linear(a_dim, k)
        w_g = nn.Linear(b_dim, k)
        
        s_ -> (1, a_dim)
        g_ -> (1, b_dim)
        are the learned co-attention feature vectors that depict how words in
        the source tweet are attended by users who interact with one another
        """
        tanh = nn.Tanh()
        
        f = tanh(torch.bmm(self.w_sg(a), b.transpose(1,2)))
        
        h_s = tanh(self.w_s(a) 
                      + torch.bmm(self.w_g(b).transpose(1,2), f.transpose(1,2)).transpose(1,2))
        h_g = tanh(self.w_g(b) 
                      + torch.bmm(self.w_s(a).transpose(1,2), f).transpose(1,2))
        
        self.seq_len_weights = h_g
        
        a_s = F.softmax(self.w_hs(h_s))
        a_g = F.softmax(self.w_hg(h_g))
        
        s_ = torch.zeros((a.shape[0], 1, a.shape[2])).cuda()
        for i in range(a.shape[1]):
            s_ += torch.bmm(a[:, i].unsqueeze(1).transpose(1,2),a_s[:, i].unsqueeze(1).transpose(1,2)).transpose(1,2)
        g_ = torch.zeros((b.shape[0], 1, b.shape[2])).cuda()
        for i in range(b.shape[1]):
            g_ += torch.bmm(b[:, i].unsqueeze(1).transpose(1,2),a_g[:, i].unsqueeze(1).transpose(1,2)).transpose(1,2)
...
class Classifier(nn.Module):
    def __init__(self, config):
        ...
        self.co_attn_1 = Co_Attention(a_dim_1, b_dim_1, k_1)
        self.co_attn_2 = Co_Attention(a_dim_2, b_dim_2, k_2)
            
        if self.config.use_addn or True:
            layers1 = [_linear(50, 128, config.init_scalar),
                      nn.ReLU(),
                      nn.Dropout(0.3),
                      _linear(128, 256, config.init_scalar),
                      nn.ReLU()]
            layers2 = [_linear(50, 128, config.init_scalar),
                      nn.ReLU(),
                      nn.Dropout(0.3),
                      _linear(128, 256, config.init_scalar),
                      nn.ReLU()]
            layers3 = [_linear(config.addn_dim, 64, config.init_scalar),
                      nn.ReLU(),
                      nn.Dropout(0.3),
                      _linear(64, 256, config.init_scalar),
                      nn.ReLU()]
            layers4 = [_linear(a_dim_1 + b_dim_1 + 100 + 256,
                               1024, config.init_scalar),
                      nn.ReLU(),
                      nn.Dropout(0.3)]
    ...
    def attention_net(self, a, b):
        """
        a -> (bsz, seq_len, hid_siz*2)
        b -> (bsz, total_addn_size)
        torch.bmm takes two 3-D Matrices as input, mat1 (b, n, m) and mat2 (b, m, p), 
        to give an output matrix of size (b, n, p).
        self.weights1 -> (bsz, seq_len)
        self.weights2 -> (bsz, seq_len, 1)
        final output from this function (torch.bmm(a.transpose(1,2), self.weights2).squeeze(2)) -> **(bsz, hid_siz*2)**
        """
    ...
    def forward(self, x, y, z, final_state, graph_embs):
        """
        x (hidden state outputs of the LSTM) -> (bsz, seq_len, hid_siz*2)
        y (additional input vector) -> (bsz, total_size)
        z (external knowledge) -> (bsz, 2, sent_emb_size)
        final_state (final output state from LSTM) -> (bsz, hid_siz*2)
        graph_embs (both tweet and user graph embeddings) -> (bsz, 50)
        
        co_attn_1 -> b/w x & z
        co_attn_2 -> b/w graph_embs_1 & graph_embs_2
        """
        
        # print(x.shape, y.shape, z.shape, graph_embs.shape)
        
        z_, x_ = self.co_attn_1(z, x)
        
        z_ = z_.squeeze(1)
        x_ = x_.squeeze(1)
        
        # graph_embs_1, graph_embs_2 = self.co_attn_2(graph_embs[:, :50], graph_embs[:, 50:])
        
        graph_embs_1, graph_embs_2 = graph_embs, graph_embs
        
        graph_embs_1 = graph_embs_1.squeeze(1)
        graph_embs_2 = graph_embs_2.squeeze(1)
        
        y1 = self.addn_model3(y.float())
        
        sent_output = torch.cat((z_, x_, 
                                 graph_embs_1, graph_embs_2, 
                                 y1), dim=1)
...
class LstmPadding(object):
    def __init__(self, sent, sent_len, config):
        ...
        self.config = config
        # print(batch_utils.LONG_TYPE)
        idx_sort = torch.from_numpy(np.array(idx_sort)).type(batch_utils.LONG_TYPE)
...
class LSTMEncoder(nn.Module):
    ...
    def forward(self, embedded, batch, *args, **kwargs):
        # print("Forward 1: ", embedded.shape, batch.sent_len)
...
def at_loss(embedder, encoder, clf, batch, addn_batch, ek_batch, graph_embs_batch, perturb_norm_length=5.0):
    embedded = embedder(batch)
    embedded.retain_grad()
    one, two = encoder(embedded, batch)
    ce = F.cross_entropy((clf(one, addn_batch, ek_batch, two, graph_embs_batch)), batch.labels)
    ce.backward()
    ...
    loss = F.cross_entropy(clf(one, addn_batch, ek_batch, two, graph_embs_batch), batch.labels)
    return loss
...
def vat_loss(embedder, encoder, clf, batch, addn_batch, ek_batch, graph_embs_batch, perturb_norm_length=5.0,
    ...
    for ip in range(Ip):
        ...
        p_d_logit = clf(one, addn_batch, ek_batch, two, graph_embs_batch)
    ...
    p_adv_logit = clf(one, addn_batch, ek_batch, two, graph_embs_batch)
